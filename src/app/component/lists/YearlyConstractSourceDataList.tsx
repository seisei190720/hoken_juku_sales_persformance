...
const YearlyConstractSourceDataList: FC<Props> = ({
  title,
  values,
  columnHeaders,
}) => {
            ...
            <TableBody>
              {values.map((v, idx) => (
                <StyledTableRow key={`${idx}_row`}>
                  ...
                  <TableCell key={`${idx}_life_${v.生保}`} align="right">
                    {v.生保.toLocaleString()}
                  </TableCell>
                  <TableCell key={`${idx}_nonlife_${v.損保}`} align="right">
                    {v.損保.toLocaleString()}
                  </TableCell>
                  <TableCell key={`${idx}_all_${v.合計}`} align="right">
                    {v.合計.toLocaleString()}
                  </TableCell>
                </StyledTableRow>
              ))}
...
export default YearlyConstractSourceDataList;
